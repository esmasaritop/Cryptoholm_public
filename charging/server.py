...
def get_ip_from_interface(interface_name: str) -> str:
    # netifaces yoksa veya interface adı boşsa localhost'a düş
    if not interface_name or netifaces is None:
        return "127.0.0.1"
    try:
        addrs = netifaces.ifaddresses(interface_name)
        return addrs[netifaces.AF_INET][0]['addr']
    except Exception:
        # Windows'ta tipik: 'eth0' yok → fallback
        return "127.0.0.1"
...
def configuration():
    # iface verilmişse dene; yoksa dokunma (YAML'daki ip kalsın)
    interface = args.iface if args.iface is not None else ''
    addr = None
    try:
        addr = load_address(interface=interface) if interface else None
    except Exception:
        addr = None
    ...
    # SADECE geçerli bir adres varsa ip'yi güncelle
    if addr:
        config['ip'] = addr  # Windows’ta çoğu zaman None döner, bu yüzden genelde YAML’daki ip korunur
    ...
    if args.max_connected is not None:
...
async def main():
    ...
    print(f"[server] Binding IP={IP}, ports={PORT0}-{PORT7}")
...
async def on_operator(websocket, path):
    async for message in websocket:
        if message == "list":
            ...
            for cp_id, cp_ws, version in connected_clients:
                if cp_id == serial:
                    res = await cp_ws.send_set_network(version= version, slot=int(variables[0]), data=data201.NetworkConnectionProfileType(ocpp_version='OCPP16' if version == 'v1.6' else 'OCPP20', ocpp_transport= "JSON", cpp_csms_url=f"ws://{IP}:{(PORT0 if version=='v1.6' else PORT4)}/", message_timeout=30, security_profile=int(variables[1]), ocpp_interface=enums201.OCPPInterfaceType.wireless0.value))
                    if res:
